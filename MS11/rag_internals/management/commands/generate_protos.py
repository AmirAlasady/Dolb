# MS11/rag_internals/management/commands/generate_protos.py
...
class Command(BaseCommand):
    help = 'Generates and fixes Python gRPC code from .proto files for the RAG Service.'
    requires_system_checks = [] # Allows this command to run without a full Django app check

    def handle(self, *args, **options):
        # Define paths relative to the Django project's base directory
        base_dir = Path(__file__).resolve().parent.parent.parent.parent
        proto_path = base_dir / 'rag_internals' / 'protos'
        output_path = base_dir / 'rag_internals' # Generate directly into the app
